...
async def search_articles(
    ...
    """
    logger.info(f"用户 {current_user.id} 正在搜索文章，关键词: '{q}'")
    if not q.strip():
...
async def read_article(
    ...
    如果用户首次查看，文章会自动标记为已读。
    """
    logger.info(f"用户 {current_user.id} 正在请求文章详情, ID: {article_id}")
...
async def update_article_status_endpoint(
    ...
    """
    logger.info(f"用户 {current_user.id} 正在更新文章 (ID: {article_id}) 的状态: {status_update.model_dump(exclude_unset=True)}")
    ...
    logger.success(f"成功为用户 {current_user.id} 更新了文章 (ID: {article_id}) 的状态。")
    return result
...
async def mark_all_as_read(
    ...
    这是一个高效的批量操作。
    """
    logger.info(f"用户 {current_user.id} 请求将 Feed ID: {feed_id or '所有'} 的文章标记为已读。")
