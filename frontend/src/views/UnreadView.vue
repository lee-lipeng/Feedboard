  ...
  <MainLayout>
    <div class="unread-container">
      ...
      <div v-if="isLoading && !initialDataLoaded" class="loading-container">
        ...
        <p class="loading-text">正在加载未读文章...</p>
      </div>
...
// 获取未读文章
const fetchUnreadArticles = async (page = 1) => {
  try {
    const skip = (page - 1) * pageSize.value;
    // 使用 is_read: false 过滤参数
...
const changePage = async (page: number) => {
  ...
  await fetchUnreadArticles(page);
};
...
const backToList = () => {
  ...
  // 返回时刷新列表，确保已读的文章被移除
  fetchUnreadArticles(currentPage.value);
};

// 处理文章更新（例如，从详情页返回后，文章可能已读）
const handleArticleUpdate = async () => {
  await fetchUnreadArticles(currentPage.value);
