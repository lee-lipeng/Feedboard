  ...
  <MainLayout>
    <div class="read-later-container">
      ...
      <div v-if="isLoading && !initialDataLoaded" class="loading-container">
        ...
        <p class="loading-text">正在加载稍后阅读文章...</p>
      </div>
...
// 获取稍后阅读文章
const fetchReadLaterArticles = async (page = 1) => {
  try {
    ...
    console.error('获取稍后阅读文章失败:', error);
...
const changePage = async (page: number) => {
  ...
  await fetchReadLaterArticles(page);
};
...
const backToList = () => {
  ...
  // 返回列表时刷新数据,确保列表是最新的
  fetchReadLaterArticles(currentPage.value);
};
...
const handleArticleUpdate = async (articleId?: number) => {
  ...
  if (articleId && currentView.value === 'detail') {
    ...
    // 如果文章存在且不再是稍后阅读状态,不自动返回列表,只更新本地数据
    if (article && !article.read_later) {
      // 不再自动跳转到列表页
    ...
    // 重新获取当前页的数据
    await fetchReadLaterArticles(currentPage.value);
...
// 组件挂载时获取数据
onMounted(async () => {
  await fetchReadLaterArticles(1);
});
</script>
